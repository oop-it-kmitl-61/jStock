 ...
 * @author Passawit
 */
public class DashboardDayUtil {

    SimpleDateFormat dayD = new SimpleDateFormat("dd");
    private String day = dayD.format(new Date());
    ...
    public DashboardDayUtil() {

        while (curs.hasNext()) {
            DBObject t = curs.next();
            DBObject date = (DBObject) t.get("date");
            if (check((String) date.get("day"), (String) date.get("month"), (String) date.get("year"))) {
    ...
    private boolean check(String day, String month, String year) {
